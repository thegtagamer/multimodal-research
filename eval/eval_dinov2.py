...
# **Dataset Class for DINOv2 (Added in Eval)**
class DisasterImageDataset(Dataset):
    def __init__(self, data_path, processor, transform=None):
        self.image_paths = []
        self.labels = []
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            image = self.transform(image)  # Apply resizing only, NO normalization
...
image_transforms = transforms.Compose([
    transforms.Resize((224, 224)),  # ✅ Only resizing, NO normalization
    transforms.ToTensor()
])

test_dataset = DisasterImageDataset(test_directory, processor, transform=image_transforms)
...
model = AutoModelForImageClassification.from_pretrained("facebook/dinov2-large", num_labels=num_classes)
model.load_state_dict(torch.load("./models/best_dinov2_image_classifier.pth", map_location=device))  # ✅ Load best model
model.to(device)
model.eval()
